...
export default function SearchStudent() {
  const [type, setType] = useState('barcode'); const [student, setStudent] = useState(null); const [error, setError] = useState('');
  ...
  return <><div className="page-title"><div><span className="eyebrow">Instant lookup</span><h1>Search student</h1><p>Scan either identity and open the complete student profile.</p></div></div>
    <section className="search-hero"><div className="search-art"><SearchIcon /></div><div className="type-switch"><button className={type === 'barcode' ? 'active' : ''} onClick={() => changeType('barcode')}><Barcode />Barcode</button><button className={type === 'rfid' ? 'active' : ''} onClick={() => changeType('rfid')}><Radio />RFID card</button></div>
      {type === 'rfid' && <div className={`reader-status ${reader.connected ? 'connected' : 'disconnected'}`}><div>{reader.connected ? <Radio /> : <Usb />}<span><strong>{reader.connected ? 'ACR122U connected' : 'NFC reader offline'}</strong><small>{reader.connected ? 'Tap a mapped card to open the student profile automatically.' : reader.error || 'Connect the reader and restart the backend.'}</small></span></div><b>{reader.connected ? 'Waiting for card' : 'Offline'}</b></div>}
    ...
    {student && <StudentModal student={student} onClose={() => setStudent(null)} />}</>;
}
